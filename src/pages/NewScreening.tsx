...
async function calculateLiveRisk(
  ...
  try {
    ...
    throw new Error(
      `Risk assessment returned an invalid response (${response.status}).`,
    );
  }
  ...
  if (!response.ok) {
    const errorDetail =
      ...
      typeof responseBody.detail === 'string'
        ? responseBody.detail
        : `Risk assessment failed with HTTP ${response.status}.`;
  ...
  if (
    ...
    throw new Error(
      'Risk assessment returned an invalid response.',
    );
  }

  const result =
    responseBody as RiskApiResponse;

  if (!result.success || !result.data) {
    throw new Error(
      'Risk assessment was not successful.',
    );
  }
...
export function NewScreening() {
  ...
  const [
    liveRiskResult,
    setLiveRiskResult,
  ] = useState<RiskAssessmentResult | null>(
    null
  );

  const runner = useRef(
    createScreeningRunner()
  );
  ...
  useEffect(() => {
    ...
    setLiveTamperingResult(null);

    setLiveFaceResult(null);

    setLiveRiskResult(null);

    if (
      handoff.current?.documentFiles?.every(
        (file) =>
          file instanceof File
      )
    ) {
      addFiles(
        handoff.current.documentFiles
      );
    }

    if (
      handoff.current?.selfieFiles?.every(
        (file) =>
          file instanceof File
      )
    ) {
      addSelfies(
        handoff.current.selfieFiles
      );
    }
    ...
    useCallback(
      async () => {
        ...
        if (
          ...
          setError(
            'Please upload a document or select a demo scenario'
          );
          return;
        }
        ...
        if (
          ...
          setError(
            'Add a selfie or choose a sample pair.'
          );
          return;
        }
        ...
        try {
          let uploadedBackendFiles:
            Awaited<
              ReturnType<
                typeof uploadSelectedFiles
              >
            > = [];
          ...
          /*
           * DEMO / EXISTING SCREENING RUNNER
           */
          const template =
            scenario
              ? createDemoCase(
          ...
          if (
            ...
            if (
              ...
              setError(
                'Screening cancelled. No final result is available.'
              );
            }
          }
        } catch (err) {
  ...
  const displayedTamperingResult =
    liveTamperingResult ??
    tamperingResults;

  const displayedFaceResult =
    liveFaceResult ??
    faceResults;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-text">
                Multi-step AI-powered document screening workflow
              </p>
            </div>
          </div>
      ...
      <FadeIn delay={0.1}>
        <StepProgress
          steps={screeningSteps.map(
            (s) => s.label
          )}
          currentStep={
            currentStepIndex
          }
          completedSteps={completedSteps.map(
            (s) =>
              STEP_ORDER.indexOf(
                s
              )
          )}
          statuses={STEP_ORDER.map(
            (step) =>
              stepStatuses[step]
          )}
          className="mb-6"
      ...
      <FadeIn delay={0.15}>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="min-w-0 lg:col-span-2 space-y-6">
            ...
            >
              <TabsList
                className="flex gap-1 bg-panel-secondary p-1 rounded-lg"
                aria-label="Screening steps"
              >
                {screeningSteps.map(
                  (
                    step,
                    ...
                    <CardContent>
                      <div className="space-y-3 mb-5">
                        <Badge variant="warning">
                          SIH DEMO — No government database access
                        </Badge>

                        <p className="text-sm text-muted-text">
                          Real uploads now run through the backend OCR, validation and image-forensics stages. Demo scenarios still use simulated screening data.
                        </p>
                        ...
                        <label className="block label">
                          Selfie input (PNG or JPEG)

                          <input
                            className="input mt-1"
                            type="file"
                            accept={SELFIE_TYPES.join(
                              ','
                            )}
                            disabled={
                              isProcessing
                        ...
                        <p className="text-sm text-muted-text">
                          {selfie.files[0]
                            ...
                            activeScenario &&
                            !files.length
                              ? 'Using the selected sample selfie paired with the sample document.'
                              : 'Add a selfie for an uploaded document.')}
                        </p>
                        ...
                        <label className="block label">
                          ...
                          >
                            ...
                            <option value="spoof">
                              Spoof attempt — simulated
                            </option>

                            <option value="unknown">
                              Inconclusive — simulated
                            </option>
                          </select>
                        </label>
                      </div>
                ...
                <FadeIn>
                  ...
                  extractedData ? (
                    ...
                    <div className="text-center py-12 text-muted-text">
                      ...
                      <p>
                        Run screening to extract OCR data
                      </p>
                    </div>
                  )}
                </FadeIn>
                ...
                <FadeIn>
                  {validationResults ? (
                    ...
                    <div className="text-center py-12 text-muted-text">
                      ...
                      <p>
                        Run screening to validate document
                      </p>
                    </div>
                  )}
                </FadeIn>
              ...
              <TabContent value="forensics">
                <FadeIn>
                  {displayedTamperingResult ? (
                    <TamperingResultsView
                      results={
                        displayedTamperingResult
                      }
                    />
                    ...
                    <div className="text-center py-12 text-muted-text">
                      ...
                      <p>
                        Run screening for tampering analysis
                      </p>
                    </div>
                  )}
                </FadeIn>
                ...
                <FadeIn>
                  {displayedFaceResult ? (
                    ...
                    <div className="text-center py-12 text-muted-text">
                      <AnimatedStatus
                        status={
                          stepStatuses.face_verification
                        }
                      >
                        {
                          stepStatuses.face_verification
                      ...
                      <p>
                        Run screening for face verification
                      </p>
                    </div>
                  )}
                </FadeIn>
                ...
                <FadeIn>
                  {displayedRiskResult ? (
                    ...
                    <div className="text-center py-12 text-muted-text">
                      ...
                      <p>
                        Run screening for risk assessment
                      </p>
                    </div>
                  )}
                </FadeIn>
                  ...
                  displayedRiskResult ? (
                    ...
                    <div className="text-center py-12 text-muted-text">
                      ...
                      <p className="text-muted-text mt-1">
                        Results will appear here after screening completes
                      </p>
                    </div>
                  )}
                </FadeIn>
          ...
          {scenarios.map(
            (scenario) => (
              <button
                ...
                className={cn(
                  'w-full text-left p-4 rounded-lg border border-border hover:border-primary-accent/50 transition-colors flex items-center gap-4',
                  activeScenario ===
                    scenario.id &&
        ...
        {documents.map(
          (doc) => (
            <button
              ...
              className={cn(
                'flex-shrink-0 p-1 rounded border-2 transition-colors',
                selectedDoc.id ===
                  doc.id
      ...
      <CardContent>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <div className="space-y-3 max-h-[400px] overflow-y-auto">
              {data.extractedFields?.map(
                (field: any) => (
                  <div
                    key={
                      field.key
                    }
                    className="bg-panel-secondary rounded-lg p-3"
                  >
        ...
        {data.mrz && (
          <div className="mt-6 pt-6 border-t border-border">
            ...
            <h4 className="font-medium text-text mt-4 mb-3">
              MRZ Parsed Fields
            </h4>

            <div className="grid grid-cols-2 gap-2 text-sm">
              {Object.entries(
                data.mrz.parsedFields
              ).map(
                ([
                  key,
                  value,
          ...
          {results.checks?.map(
            (
              ...
              <div
                ...
                className={cn(
                  'flex items-center gap-3 p-3 bg-panel-secondary rounded-lg border',
                  check.status ===
                    'pass' &&
                    'border-success/30',
                  check.status ===
      ...
      <CardContent>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div>
            ...
            <div className="bg-panel-secondary rounded-lg p-4 min-h-[300px] flex items-center justify-center relative">
              <div className="text-center">
                <Shield className="w-14 h-14 mx-auto mb-3 text-primary-accent" />
                ...
                <p className="text-sm text-muted-text mt-1">
                  ELA, noise, edges, texture and metadata signals analyzed
                </p>
              </div>
          ...
          <div>
            ...
            <div className="bg-panel-secondary rounded-lg p-4 border border-primary-accent/30 mb-4">
              <div className="flex items-center justify-between mb-2">
                <div>
                  ...
                  <p className="text-xs text-muted-text">
                    Lower score indicates fewer detected forensic anomalies.
                  </p>
                </div>
            ...
            <div className="bg-panel-secondary rounded-lg p-4 mb-4">
                ...
                (
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        'p-3 bg-panel-secondary rounded-lg border',
                        variant ===
                          'success' &&
                          'border-success/30',
                        variant ===
      ...
      <CardContent>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-muted-text">
              Liveness:{' '}
              {results.presentedFace?.livenessStatus
                ?.replace(
                  '_',
                  ' '
                ) ??
    ...
    <Card padding="lg">
      ...
      <CardContent>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="flex flex-col items-center justify-center py-8">
            ...
            <p className="text-center text-muted-text">
              AI-assisted risk scoring based on multi-factor analysis
            </p>
          </div>
        ...
        <div className="mt-8 pt-6 border-t border-border">
          <h4 className="font-medium text-text mb-4">
            Explainable AI — Why was this case flagged?
          </h4>
...
function FinalResultView({
  ...
  const tamperingStatus =
    tamperingResults?.overallStatus ??
    'clean';
          ...
          <CardContent className="pt-0">
            ...
            <FadeIn y={8}>
              ...
              >
                ...
                >
                  {riskResult.recommendation.replace(
                    '_',
                    ' '
                  ).toUpperCase()}
                </p>
                      ...
                      (
                        ...
                        >
                          <div className="p-3 bg-panel-secondary rounded-lg border border-border/50 flex items-start gap-3">
                            <span className="text-primary-accent font-mono mt-0.5">
                              {i +
                                1}
                              .
                            </span>
