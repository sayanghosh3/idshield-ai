...
type ScreeningAction =
  | { type: 'SET_CURRENT_CASE'; payload: ScreeningCase | null }
  | { type: 'SET_UPLOADED_DOCUMENTS'; payload: DocumentFile[] }
  | { type: 'ADD_DOCUMENT'; payload: DocumentFile }
  | { type: 'REMOVE_DOCUMENT'; payload: string }
  | { type: 'SET_EXTRACTED_DATA'; payload: OCRResult | null }
  | { type: 'SET_VALIDATION_RESULTS'; payload: ValidationResult | null }
  | { type: 'SET_TAMPERING_RESULTS'; payload: TamperingResult | null }
  | { type: 'SET_FACE_RESULTS'; payload: FaceVerificationResult | null }
  | { type: 'SET_RISK_RESULT'; payload: RiskResult | null }
  | { type: 'ADD_AUDIT_EVENT'; payload: AuditEvent }
  | { type: 'SET_AUDIT_EVENTS'; payload: AuditEvent[] }
  | { type: 'SET_SCREENING_STATUS'; payload: ScreeningStatus }
  | { type: 'SET_CURRENT_STEP'; payload: ScreeningStep }
  | { type: 'SET_PROGRESS'; payload: { progress: number; message: string } }
  | { type: 'SET_ERROR'; payload: string | null }
  | { type: 'RESET_SCREENING' }
  | { type: 'LOAD_DEMO_SCENARIO'; payload: ScreeningCase };
...
function screeningReducer(state: ScreeningState, action: ScreeningAction): ScreeningState {
  switch (action.type) {
    ...
    case 'REMOVE_DOCUMENT':
      return { ...state, uploadedDocuments: state.uploadedDocuments.filter(d => d.id !== action.payload) };
    ...
    case 'SET_TAMPERING_RESULTS':
      return { ...state, tamperingResults: action.payload };
    case 'SET_FACE_RESULTS':
      return { ...state, faceResults: action.payload };
    case 'SET_RISK_RESULT':
      return { ...state, riskResult: action.payload };
...
interface ScreeningContextType extends ScreeningState {
  ...
  setTamperingResults: (results: TamperingResult | null) => void;
  setFaceResults: (results: FaceVerificationResult | null) => void;
  ...
  resetScreening: () => void;
  loadDemoScenario: (scenarioCase: ScreeningCase) => void;
}
...
export function ScreeningProvider({ children }: { children: ReactNode }) {
  ...
  const actions = useMemo(() => ({
    setCurrentCase: (payload: ScreeningCase | null) => dispatch({ type: 'SET_CURRENT_CASE', payload }),
    setUploadedDocuments: (payload: DocumentFile[]) => dispatch({ type: 'SET_UPLOADED_DOCUMENTS', payload }),
    addDocument: (payload: DocumentFile) => dispatch({ type: 'ADD_DOCUMENT', payload }),
    removeDocument: (payload: string) => dispatch({ type: 'REMOVE_DOCUMENT', payload }),
    setExtractedData: (payload: OCRResult | null) => dispatch({ type: 'SET_EXTRACTED_DATA', payload }),
    setValidationResults: (payload: ValidationResult | null) => dispatch({ type: 'SET_VALIDATION_RESULTS', payload }),
    setTamperingResults: (payload: TamperingResult | null) => dispatch({ type: 'SET_TAMPERING_RESULTS', payload }),
    setFaceResults: (payload: FaceVerificationResult | null) => dispatch({ type: 'SET_FACE_RESULTS', payload }),
    setRiskResult: (payload: RiskResult | null) => dispatch({ type: 'SET_RISK_RESULT', payload }),
    addAuditEvent: (payload: AuditEvent) => dispatch({ type: 'ADD_AUDIT_EVENT', payload }),
    setAuditEvents: (payload: AuditEvent[]) => dispatch({ type: 'SET_AUDIT_EVENTS', payload }),
    setScreeningStatus: (payload: ScreeningStatus) => dispatch({ type: 'SET_SCREENING_STATUS', payload }),
    setCurrentStep: (payload: ScreeningStep) => dispatch({ type: 'SET_CURRENT_STEP', payload }),
    setProgress: (progress: number, message: string) => dispatch({ type: 'SET_PROGRESS', payload: { progress, message } }),
    setError: (payload: string | null) => dispatch({ type: 'SET_ERROR', payload }),
    resetScreening: () => dispatch({ type: 'RESET_SCREENING' }),
    loadDemoScenario: (payload: ScreeningCase) => dispatch({ type: 'LOAD_DEMO_SCENARIO', payload }),
  }), []);

  return (
    <ScreeningContext.Provider value={{ ...state, ...actions }}>
      {children}
